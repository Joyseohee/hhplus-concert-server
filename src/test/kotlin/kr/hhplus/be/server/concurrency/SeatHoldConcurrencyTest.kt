...
class SeatHoldConcurrencyTest @Autowired constructor(
	...
	given("좌석 점유가 예약 시점에 만료된 경우") {
		`when`("스케줄러로 seatHold를 만료시키고 예약을 시도하면") {
			then("예외가 발생한다") {
				val concert = concertRepository.save(
					Concert.create(
						title = "테스트콘서트",
	...
	given("동시성 테스트 - seatHold 만료와 예약 경쟁") {
		`when`("여러 스레드가 동시에 예약을 시도하고, 그 사이 스케줄러가 seatHold를 만료시키는 경우") {
			then("seatHold가 만료된 후 예약을 시도한 스레드는 예외가 발생한다") {
				val concert = concertRepository.save(
					Concert.create(
						title = "동시성콘서트",
						venue = "테스트홀",
